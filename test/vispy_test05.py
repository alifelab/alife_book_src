...
void main()
{
    ...
    if(length(gl_PointCoord.xy - vec2(0.5)) < 0.4) {
        gl_FragColor = vec4(0, 0, 0, 0);
    } else if(length(gl_PointCoord.xy - vec2(0.5)) < 0.5) {
...
class Canvas(app.Canvas):
    ...
    def on_draw(self, event):
        ...
        # Draw
        self._program['u_time'] = (time.time() - self._starttime)/10
        self._program.draw('points')

        # New explosion?
        if time.time() - self._starttime > 15:
            self._new_explosion()
